...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    //val resultDataframe = spark.sql("select rectangle, count(*) from joinResult group by rectangle order by rectangle")
    //return resultDataframe
    val resultdf = joinDf.groupBy("rectangle").count()
    val outputdf = resultdf.coalesce(1)
    return outputdf
     // YOU NEED TO CHANGE THIS PART
  }
